...
def embed_astro(markers, **kwargs):
    return f''' <iframe width=100% height=640 src="{create_astro_galmap_url(markers, **kwargs)}"></iframe> '''
...
```{python}
fig_url = create_astro_galmap_url(
...
We now group the system coordinates to the cluster groups we found and calculate the lines we saw for each group
...
guardian_projected_clusters_json_pins = "guardian-projected-clusters.json"
with open(guardian_projected_clusters_json_pins, 'wt') as of:
        json.dump(dict(markers=[
        dict(
            pin='white',
            text=f"cluster {cc}, distance {round(distance_point_to_line(cc, **gsystem_lines[l]),1)}\ncentre \t{cc}\nprojected \t{project_point_on_line(cc, **gsystem_lines[l])}",
...
```{python}
fig_url = create_astro_galmap_url(
    markers = [guardian_projected_clusters_json_pins, guardian_clusters_json_pins],
    layer = 'heatmap')

Markdown(f"""
{embed_astro( [guardian_projected_clusters_json_pins, guardian_clusters_json_pins], layer='heatmap')}
...
img = plt.imread("visited-systems-indexedregions.jpg")
fig, ax = plt.subplots()
ax.imshow(img)
```
